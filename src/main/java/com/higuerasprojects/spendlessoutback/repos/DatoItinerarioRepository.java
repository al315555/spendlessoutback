...
public interface DatoItinerarioRepository extends JpaRepository<DatoItinerario, Long> {
	...
	@Query(value="SELECT * FROM tdato_itinerario u WHERE u.ubicacion_nombre=?1 AND u.ubicacion_lon=?3 AND u.ubicacion_lat=?2 ORDER BY CASE WHEN 'true'=?4 THEN u.precio_total ASC ELSE u.precio_total DESC", nativeQuery=true)
	...
	@Query(value="SELECT * FROM tdato_itinerario u WHERE u.id_user=?1 ORDER BY CASE WHEN 'true'=?2 THEN u.precio_total ASC ELSE u.precio_total DESC ", nativeQuery=true)
